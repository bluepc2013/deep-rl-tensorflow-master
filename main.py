...
# Deep q Network
flags.DEFINE_boolean('use_gpu', False, 'Whether to use gpu or not. cpu use NHWC and gpu use NCHW for data_format')
...
# Training
# tip 1: loss = sum [ r+discount_factor*Q(s',a')-Q(s,a) ]^2, @max_delta and @min_delta is used to clip \
# [r+discount_factor*Q(s',a')-Q(s,a)] .
# tip 2: this algorithm have a chance to take a random action, the probability drop from @ep_start to @ep_end.
...
# Timer
flags.DEFINE_integer('t_train_freq', 4, '') #training the net every 4 times.

# Below numbers will be multiplied by scale
flags.DEFINE_integer('scale', 1000, 'The scale for big numbers')
flags.DEFINE_integer('memory_size', 100, 'The size of experience memory (*= scale)') # unit is frame.
...
flags.DEFINE_float('t_learn_start', 5, 'The time when to begin training for the first time (*= scale)') 
...
def calc_gpu_fraction(fraction_string):
  ...
  fraction = 1 / (num - idx + 1) # ?为什么要这么换算
  print " [*] GPU : %.4f" % fraction
  return fraction

conf = flags.FLAGS

if conf.agent_type == 'DQN':  # have not Dueling or DDQN ?
...
def main(_):
  # preprocess
  conf.observation_dims = eval(conf.observation_dims) # string to int, ["84","84"] --> [84,84]

  # record configuration information into @conf, @conf will be used as a configuration variable.
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    # choice environment
    if any(name in conf.env_name for name in ['Corridor', 'FrozenLake']) : # any( () for name in ['C','F'] )
